...
class Tree
  ...
  def depth_first_search(target_key)
    stack = [self]  # start with this tree/root in stack
    until stack.empty?
      # print out key of each entry in stack to help with debugging
      p stack.map { |node| "#{node.key}" }

      # pop last entry in the stack
      current_node = stack.pop

      # check if this node has the right key (and return early if so)
  ...
  # Returns nil if no such node is found.
  #
  # This version keeps track of "visited" nodes, whih is important for searching
  # more general graphs (instead of just trees).
  def depth_first_search_visiting(target_key)
